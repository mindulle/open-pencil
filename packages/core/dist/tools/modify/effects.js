import { DEFAULT_SHADOW_COLOR, TRANSPARENT } from "../../constants.js";
import { parseColor } from "../../color/index.js";
import { defineTool, nodeNotFound } from "../schema.js";
//#region src/tools/modify/effects.ts
const setEffects = defineTool({
	name: "set_effects",
	mutates: true,
	description: "Set effects on a node (drop shadow, inner shadow, blur). Pass an array or a single effect.",
	params: {
		id: {
			type: "string",
			description: "Node ID",
		...
		color: {
			type: "color",
			description: "Shadow color (hex). Ignored for blur."
		},
		offset_x: {
			type: "number",
	...
	execute: (figma, args) => {
		...
		const effect = {
			...
			color,
			offset: {
				x: isBlur ? 0 : args.offset_x ?? 0,
				y: isBlur ? 0 : args.offset_y ?? 4
			},
