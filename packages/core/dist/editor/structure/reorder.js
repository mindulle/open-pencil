...
//#region src/editor/structure/reorder.ts
function createStructureReorderActions(ctx) {
	function doReorderChild(nodeId, parentId, insertIndex) {
	...
	function reorderInAutoLayout(nodeId, parentId, insertIndex) {
	...
	function reorderChildWithUndo(nodeId, newParentId, insertIndex) {
		const node = ctx.graph.getNode(nodeId);
		if (!node) return;
		const origParentId = node.parentId ?? ctx.state.currentPageId;
	...
	function applyChildOrder(parentId, childIds) {
	...
	function moveSelectionInZOrder(label, reorder) {
	...
	function moveAdjacent(childIds, selectedIds, direction) {
	...
	function bringForward() {
		moveSelectionInZOrder("Bring forward", (childIds, selectedIds) => moveAdjacent(childIds, selectedIds, "forward"));
	}
	function sendBackward() {
		moveSelectionInZOrder("Send backward", (childIds, selectedIds) => moveAdjacent(childIds, selectedIds, "backward"));
	}
	function bringToFront() {
		moveSelectionInZOrder("Bring to front", (childIds, selectedIds) => [...childIds.filter((id) => !selectedIds.has(id)), ...childIds.filter((id) => selectedIds.has(id))]);
	}
	function sendToBack() {
		moveSelectionInZOrder("Send to back", (childIds, selectedIds) => [...childIds.filter((id) => selectedIds.has(id)), ...childIds.filter((id) => !selectedIds.has(id))]);
	}
	return {
		reorderInAutoLayout,
