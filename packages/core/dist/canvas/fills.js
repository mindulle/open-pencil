...
import { makeSmoothRRectPath, nodeHasSmoothCorners } from "./shapes.js";
//#region src/canvas/fills.ts
function paintFills(r, fills, node, graph, draw, options = {}) {
	for (let index = 0; index < fills.length; index++) {
		const fill = fills[index];
		if (!fill.visible) continue;
		const fillIndex = options.bindToNodeFills === false ? -1 : index;
		if (!(options.patternStack ? applyFill(r, fill, node, graph, fillIndex, options.patternStack) : r.applyFill(fill, node, graph, fillIndex))) continue;
...
function drawVectorMultiStyleFills(r, canvas, node, graph, patternStack) {
	if (node.type !== "VECTOR" || !node.fillGeometry.some((geometry) => geometry.fills?.length)) return false;
	...
	for (let index = 0; index < node.fillGeometry.length; index++) {
		...
		paintFills(r, usesPathFills ? geometry.fills ?? [] : node.fills, node, graph, () => canvas.drawPath(path, r.fillPaint), {
			bindToNodeFills: !usesPathFills,
			patternStack
		});
	}
	return true;
}
function drawNodeFill(r, canvas, node, rect, hasRadius, fill) {
	switch (node.type) {
		case "VECTOR": {
			const fg = r.getFillGeometry(node);
			if (fg) for (const p of fg) canvas.drawPath(p, r.fillPaint);
			else {
				const vps = r.getVectorPaths(node);
				if (vps) for (const vp of vps) canvas.drawPath(vp, r.fillPaint);
			}
			break;
		}
		case "ELLIPSE": {
			const fg = r.getFillGeometry(node);
			if (fg) for (const p of fg) canvas.drawPath(p, r.fillPaint);
			else if (node.arcData) r.drawArc(canvas, node, r.fillPaint);
			else canvas.drawOval(rect, r.fillPaint);
			break;
		}
		case "TEXT":
		...
		default: if (nodeHasSmoothCorners(node)) {
			...
			canvas.drawPath(path, r.fillPaint);
			path.delete();
		} else if (hasRadius) canvas.drawRRect(r.makeRRect(node), r.fillPaint);
		else canvas.drawRect(rect, r.fillPaint);
	}
}
function applyFill(r, fill, node, graph, fillIndex = 0, patternStack = /* @__PURE__ */ new Set()) {
	r.fillPaint.setShader(null);
	if (fill.type === "SOLID") {
	...
	if (fill.type === "IMAGE" && fill.imageHash) return r.applyImageFill(fill, node, graph);
...
function patternAlignmentOffset(alignment, gap, sourceSize, axis) {
	if (alignment === "CENTER") return axis === "x" ? -gap / 2 : -sourceSize / 2;
	if (alignment === "END") return -gap;
	return 0;
}
function patternTileLayout(source, fill) {
	const scale = fill.scale && fill.scale > 0 ? fill.scale : 1;
	const spacing = fill.patternSpacing ?? {
		x: 0,
		y: 0
	};
	...
	if (fill.patternTileType === "HORIZONTAL_HEXAGONAL") positions.push({
		x: x + width / 2,
		y: y + height / 2
	});
	else if (fill.patternTileType === "VERTICAL_HEXAGONAL") positions.push({
		x: x + width / 2,
		y: y - height / 2
	});
	return {
...
function recordPatternSource(r, source, graph, layout, patternStack) {
	...
	for (const position of layout.positions) {
		...
		if (!drawVectorMultiStyleFills(r, canvas, source, graph, patternStack)) paintFills(r, source.fills, source, graph, (sourceFill) => drawNodeFill(r, canvas, source, rect, hasRadius, sourceFill), { patternStack });
		canvas.restore();
	}
...
function resolvePatternSource(graph, sourceId) {
	...
	for (const node of graph.getAllNodes()) if (node.source.id === sourceId) return node;
	return null;
}
function applyPatternFill(r, fill, node, graph, patternStack) {
	...
	const shader = picture.makeShader(r.ck.TileMode.Repeat, r.ck.TileMode.Repeat, r.ck.FilterMode.Linear, void 0, tileRect);
	r.fillPaint.setShader(shader);
	picture.delete();
	return true;
}
function makeGradientLocalMatrix(r, width, height, transform) {
...
function linearGradientEndpoints(width, height, transform) {
	return {
		start: {
			...
			x: transform.m02 * width,
			y: transform.m12 * height
		}
	};
}
function applyGradientFill(r, fill, node, graph) {
	const stops = fill.gradientStops;
	const t = fill.gradientTransform;
	if (!stops || !t) return;
	const colors = stops.map((s, index) => {
		const c = r.resolveFillColorInfo({
			...fill,
			type: "SOLID",
			color: s.color,
			opacity: s.color.a,
			visible: true
		}, index, node, graph).color;
	...
	if (fill.type === "GRADIENT_LINEAR") {
		...
		const endX = end.x;
		const endY = end.y;
		const shader = r.ck.Shader.MakeLinearGradient([startX, startY], [endX, endY], colors, positions, r.ck.TileMode.Clamp);
		r.fillPaint.setShader(shader);
	} else if (fill.type === "GRADIENT_RADIAL" || fill.type === "GRADIENT_DIAMOND") {
		const localMatrix = makeGradientLocalMatrix(r, w, h, t);
		const shader = r.ck.Shader.MakeRadialGradient([.5, .5], .5, colors, positions, r.ck.TileMode.Clamp, localMatrix);
		r.fillPaint.setShader(shader);
	} else if (fill.type === "GRADIENT_ANGULAR") {
		const localMatrix = makeGradientLocalMatrix(r, w, h, t);
		const shader = r.ck.Shader.MakeSweepGradient(.5, .5, colors, positions, r.ck.TileMode.Clamp, localMatrix);
		r.fillPaint.setShader(shader);
	}
}
function makeImageFillLocalMatrix(r, fill, node, imgW, imgH) {
	...
	if ((scaleMode === "CROP" || scaleMode === "TILE") && fill.imageTransform) {
		...
		if (inverse) return r.ck.Matrix.multiply(r.ck.Matrix.scaled(node.width, node.height), inverse, r.ck.Matrix.scaled(1 / imgW, 1 / imgH));
	}
	let sx, sy, sw, sh;
	if (scaleMode === "FIT") {
	...
	return r.ck.Matrix.multiply(r.ck.Matrix.scaled(node.width / sw, node.height / sh), r.ck.Matrix.translated(-sx, -sy));
}
function applyImageFill(r, fill, node, graph) {
	...
	if (!img) {
		...
		const decoded = r.ck.MakeImageFromEncoded(data) ?? void 0;
	...
	if (scaleMode === "TILE") {
		const shader = img.makeShaderCubic(r.ck.TileMode.Repeat, r.ck.TileMode.Repeat, 1 / 3, 1 / 3, localMatrix);
		r.fillPaint.setShader(shader);
		return true;
	}
	const shader = img.makeShaderOptions(r.ck.TileMode.Clamp, r.ck.TileMode.Clamp, r.ck.FilterMode.Linear, r.ck.MipmapMode.Linear, localMatrix);
	r.fillPaint.setShader(shader);
	return true;
}
function makeArcPath(r, node) {
	...
	const sweepDeg = arc.endingAngle * (180 / Math.PI) - startDeg;
	...
	if (Math.abs(sweepDeg) >= 359.99) path.addOval(oval);
	else {
		path.moveTo(cx, cy);
		path.addArc(oval, startDeg, sweepDeg);
		path.close();
...
function drawArc(r, canvas, node, paint) {
