import { dropShadowToCSS, fillToCSS, sceneNodeSizeStyle, strokeColorToCSS, strokeToCSS } from "./browser.js";
import { encodeBase64 } from "@open-pencil/core/bytes";
import { BLACK } from "@open-pencil/scene-graph/constants";
import { colorToCSS } from "@open-pencil/core/color";
//#region src/from-scene-graph.ts
...
function alignItemsToCSS(value) {
	if (value === "CENTER") return "center";
	if (value === "MAX") return "flex-end";
	if (value === "STRETCH") return "stretch";
	if (value === "BASELINE") return "baseline";
}
function alignSelfToCSS(value) {
...
function addCornerRadii(style, node) {
	if (node.independentCorners) {
		...
		if (node.bottomRightRadius > 0) style["border-bottom-right-radius"] = `${node.bottomRightRadius}px`;
...
function styleFromTextNode(node) {
	...
	if (node.textAlignHorizontal !== "LEFT") style["text-align"] = node.textAlignHorizontal.toLowerCase();
	if (node.opacity < 1) style.opacity = String(node.opacity);
	const shadow = dropShadowToCSS(node.effects[0]);
	if (shadow) style["text-shadow"] = shadow;
	if (node.textDecoration !== "NONE") style["text-decoration-line"] = node.textDecoration === "UNDERLINE" ? "underline" : "line-through";
...
function imageSourceURL(node) {
	return node.pluginData.find((entry) => entry.pluginId === DOM_CSS_PLUGIN_ID && entry.key === IMAGE_SOURCE_URL_KEY)?.value;
}
function attrsForNode(graph, node, includeSourceIds) {
	...
	return {
		...attrs,
		src: `data:image/png;base64,${encodeBase64(bytes)}`
	};
}
function tagNameForNode(node) {
	if ((node.fills.at(0)?.type === "IMAGE" || imageSourceURL(node)) && node.childIds.length === 0) return "img";
	return "div";
}
...
function sceneGraphToDesignDocument(graph, options = {}) {
	...
	return {
		type: "document",
		sourceGraph: graph,
		children: root ? nodeChildren(graph, root).map((child) => sceneNodeToDesignNode(graph, child, resolvedOptions)).filter((child) => child !== null) : []
	};
}
//#endregion
export { sceneGraphToDesignDocument };

//# sourceMappingURL=from-scene-graph.js.map