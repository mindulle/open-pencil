...
//#region src/tools/variables/read.ts
const listVariables = defineTool({
	name: "list_variables",
...
const findVariables = defineTool({
	name: "find_variables",
	...
	execute: (figma, args) => {
		...
		variables = variables.filter((variable) => variable.name.toLowerCase().includes(args.query.toLowerCase()));
		return {
			count: variables.length,
			variables
		};
...
//#endregion
export { findVariables, getVariable, listVariables };
