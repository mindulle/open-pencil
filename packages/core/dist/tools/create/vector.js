...
import { normalizeVectorNetwork, transformVectorNetwork, validateVectorNetwork } from "@open-pencil/scene-graph";
import { parseSVGPath } from "@open-pencil/scene-graph/parse-path";
import { safeDestr } from "destr";
//#region src/tools/create/vector.ts
function parseSVGVectorPath(path) {
	const network = parseSVGPath(path);
	if (network.segments.length === 0) return { error: "SVG path data must contain at least one drawable segment" };
	const bounds = computeAccurateBounds(network);
	return {
		network: transformVectorNetwork([
			1,
			0,
			-bounds.x,
			0,
...
function parseVectorNetworkJSON(path) {
	let parsed;
	try {
	...
	return {
		network: normalizeVectorNetwork(parsed),
		size: null
	};
}
function parseVectorPath(path) {
	const trimmed = path.trim();
	if (/^[Mm]/.test(trimmed)) return parseSVGVectorPath(trimmed);
	return parseVectorNetworkJSON(trimmed);
}
const createVector = defineTool({
	name: "create_vector",
	mutates: true,
	...
	params: {
		...
		path: {
			type: "string",
			description: "SVG path data (preferred, e.g. \"M0 0 L100 0 L50 80 Z\") or VectorNetwork JSON, e.g. {\"vertices\":[{\"x\":0,\"y\":0},{\"x\":10,\"y\":0}],\"segments\":[{\"start\":0,\"end\":1}],\"regions\":[]}"
		},
		fill: {
			type: "color",
	...
	execute: (figma, args) => {
		...
		if (args.fill) node.fills = [{
			type: "SOLID",
			color: parseColor(args.fill),
			opacity: 1,
			visible: true
		}];
		if (args.stroke) figma.graph.updateNode(node.id, { strokes: [{
