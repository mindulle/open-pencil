...
//#region src/tools/vector/path.ts
function getVectorNode(figma, id) {
	const node = figma.graph.getNode(id);
	if (!node) return { error: `Node "${id}" not found` };
	if (!node.vectorNetwork) return { error: `Node "${id}" has no vector data` };
	return {
		node,
		vn: cloneVectorNetwork(node.vectorNetwork)
	};
}
const pathGet = defineTool({
	name: "path_get",
	...
	execute: (figma, { id }) => {
		...
		return vectorNetwork ? {
			id,
			vectorNetwork
		} : { error: `Node "${id}" has no vector data` };
	}
});
const pathSet = defineTool({
	name: "path_set",
	mutates: true,
	...
	params: {
		...
		path: {
			type: "string",
			description: "VectorNetwork JSON",
			required: true
		}
	},
	execute: (figma, args) => {
		if (!figma.graph.getNode(args.id)) return { error: `Node "${args.id}" not found` };
		const network = JSON.parse(args.path);
		figma.graph.updateNode(args.id, { vectorNetwork: network });
		return { id: args.id };
	}
});
const pathScale = defineTool({
	name: "path_scale",
	mutates: true,
	...
	params: {
		...
		factor: {
			type: "number",
			description: "Scale factor (e.g. 2 for double)",
			required: true
		}
	},
...
const pathFlip = defineTool({
	...
	execute: (figma, { id, axis }) => {
		...
		for (const vertex of vn.vertices) if (axis === "horizontal") vertex.x = width - vertex.x;
...
//#endregion
export { pathFlip, pathGet, pathMove, pathScale, pathSet };
