//#region src/figma-api/serialization.ts
function resolveBindings(graph, node) {
	if (Object.keys(node.boundVariables).length === 0) return void 0;
	const result = {};
	for (const [field, varId] of Object.entries(node.boundVariables)) {
		...
		result[field] = {
			...
			resolvedValue: variable ? graph.resolveVariable(varId) : void 0
		};
	}
	return result;
}
function appendTextProps(obj, n) {
...
function nodeProxyToJSON(graph, api, nodeId, maxDepth, currentDepth = 0) {
	...
	if (n.type === "TEXT") appendTextProps(obj, n);
	...
	if (children.length > 0) if (maxDepth !== void 0 && currentDepth >= maxDepth) obj.childCount = children.length;
	else obj.children = children.map((child) => api.wrapNode(child.id).toJSON(maxDepth, currentDepth + 1));
	return obj;
}
//#endregion
