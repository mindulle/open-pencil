...
//#region src/tools/structure/replace.ts
const nodeReplaceWith = defineTool({
	...
	params: {
		...
		jsx: {
			type: "string",
			description: "JSX string for the replacement",
			required: true
		}
	},
	execute: async (figma, args) => {
		...
		return {
			...
			children: results.slice(1).map((child) => ({
				id: child.id,
				name: child.name,
				type: child.type
			}))
