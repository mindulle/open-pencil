...
import { CONTAINER_TYPES, findAncestorBackground, looksLikeButton } from "./shared.js";
import { detectLayoutIssues } from "./layout-issues.js";
//#region src/tools/describe/issues.ts
const MIN_FILL_OPACITY = .15;
const MIN_STROKE_OPACITY = .2;
...
function checkEmptyIcon(node, graph, issues) {
...
function detectStructuralIssues(node, gridSize, graph, issues) {
	if (node.x % 1 !== 0 || node.y % 1 !== 0) issues.push({
		message: `Subpixel position (${node.x}, ${node.y})`,
		suggestion: `(${Math.round(node.x)}, ${Math.round(node.y)})`
	});
	if (CONTAINER_TYPES.has(node.type) && node.fills.length === 0 && node.childIds.length === 0) issues.push({ message: "Empty frame with no fill" });
	const hasVisibleFill = node.fills.some((f) => f.visible);
	const hasVisibleStroke = node.strokes.some((s) => s.visible);
	if (SHAPE_TYPES.has(node.type) && !hasVisibleFill && !hasVisibleStroke) issues.push({
		message: `${node.type} "${node.name}" has no fill and no stroke — invisible`,
		suggestion: "Add bg=\"#hex\" or stroke=\"#hex\""
	});
	checkEmptyIcon(node, graph, issues);
	if (looksLikeButton(node) && node.width < 44) issues.push({
...
function checkExcessiveNesting(node, graph, issues) {
	if (!CONTAINER_TYPES.has(node.type)) return;
	let depth = 0;
	let current = node;
...
function checkSameFillAsParent(node, graph, issues) {
	...
	if (colorDistance(nodeFill.color, parentFill.color) < 3) issues.push({
...
function checkImagePlaceholder(node, graph, issues) {
	if (!CONTAINER_TYPES.has(node.type)) return;
	if (!node.clipsContent) return;
	if (!/poster|avatar|image|thumb|photo|cover|banner/i.test(node.name)) return;
	if (node.fills.some((f) => f.visible && f.type === "IMAGE")) return;
	if (!node.childIds.some((id) => {
		const c = graph.getNode(id);
		return c?.visible && c.fills.some((f) => f.visible && f.type === "IMAGE");
	}) && !node.fills.some((f) => f.visible)) issues.push({
...
function checkNestedText(node, graph, issues) {
	if (node.type !== "TEXT") return;
	for (const childId of node.childIds) {
		const child = graph.getNode(childId);
		if (child?.type === "TEXT") issues.push({
...
function checkZeroPaddingContainer(node, graph, issues) {
	if (!CONTAINER_TYPES.has(node.type)) return;
	if (node.layoutMode === "NONE") return;
	if (node.childIds.length < 3) return;
	if (node.paddingTop + node.paddingRight + node.paddingBottom + node.paddingLeft > 0) return;
	const hasTextChild = node.childIds.some((id) => {
		const c = graph.getNode(id);
		return c?.type === "TEXT" && c.visible;
	});
	const hasFill = node.fills.some((f) => f.visible);
	if (hasTextChild && hasFill) issues.push({
		message: `"${node.name}" has text content with fill but no padding`,
		suggestion: "Add p={16} or px={16} py={12}"
	});
}
function checkButtonWithoutPadding(node, _graph, issues) {
	if (!looksLikeButton(node)) return;
	if (node.layoutMode === "NONE") return;
	if (node.paddingLeft + node.paddingRight === 0 && node.childIds.length > 0) issues.push({
		message: `Button "${node.name}" has no horizontal padding`,
		suggestion: "Add px={16} or px={12}"
	});
}
function checkFlexContainerWithoutAlignment(node, _graph, issues) {
	if (!CONTAINER_TYPES.has(node.type)) return;
	if (node.layoutMode === "NONE") return;
	if (node.childIds.length === 0) return;
	if (node.layoutMode === "HORIZONTAL" && node.counterAxisAlign === "MIN" && node.height > 60) {
		...
		if (contentH > 50 && node.childIds.length <= 5) {
			...
			if (maxChildH > 0 && maxChildH < contentH * .6) issues.push({
...
function detectVisibilityIssues(node, graph, issues) {
	for (const fill of node.fills) {
		if (!fill.visible || fill.type !== "SOLID") continue;
		if (fill.opacity < MIN_FILL_OPACITY) issues.push({
	...
	if (dist < LOW_CONTRAST_THRESHOLD) issues.push({
...
function detectRadiusIssues(node, graph, issues) {
	...
	for (const childId of node.childIds) {
		...
		if (child.cornerRadius > node.cornerRadius + RADIUS_TOLERANCE) issues.push({
			message: `"${child.name}" radius ${child.cornerRadius} > parent ${node.cornerRadius}`,
			suggestion: `Use rounded={${expectedInner}}`
		});
		else if (child.cornerRadius > expectedInner + RADIUS_TOLERANCE && expectedInner < node.cornerRadius) issues.push({
...
function detectTypographyIssues(node, graph, issues) {
	for (const childId of node.childIds) {
		const child = graph.getNode(childId);
		if (child?.type !== "TEXT" || !child.visible) continue;
		const text = child.text;
		if (text === text.toUpperCase() && text.length > 1 && /[A-ZА-ЯЁ]/.test(text) && child.fontSize > UPPERCASE_MAX_SIZE) issues.push({
...
function detectSpacingIssues(node, graph, _gridSize, issues) {
	if (node.layoutMode === "NONE") return;
	const children = node.childIds.map((id) => graph.getNode(id)).filter((c) => c?.visible === true && c.layoutPositioning !== "ABSOLUTE");
	const minPad = Math.min(node.paddingTop, node.paddingRight, node.paddingBottom, node.paddingLeft);
	if (node.itemSpacing > 0 && minPad > 0 && node.itemSpacing > minPad * 2) issues.push({
	...
	for (const v of spacingValues) if (v % SPACING_GRID !== 0) {
		issues.push({
	...
	if (flexChildren.length >= 3) {
		...
		if (new Set(paddings).size > 2) issues.push({
		...
		if (uniqueGaps.size > 2) issues.push({
...
function detectIssues(node, gridSize, graph) {
	const issues = [];
	...
	for (const issue of issues) issue.severity = classifySeverity(issue.message);
	return issues;
}
//#endregion
export { detectIssues };
