...
//#region src/tools/analyze/typography.ts
const analyzeTypography = defineTool({
	...
	params: {
		limit: {
			type: "number",
			description: "Max styles to return (default: 30)"
		},
		group_by: {
			type: "string",
	...
	execute: (figma, args) => {
		...
		if (args.group_by === "family") {
			...
			return {
				totalTextNodes,
				groups: orderBy([...byFamily.entries()], [(entry) => entry[1]], ["desc"]).map(([family, count]) => ({
					family,
					count
				}))
			};
		...
		if (args.group_by === "weight") {
			...
			return {
				totalTextNodes,
				groups: orderBy([...byWeight.entries()], [(entry) => entry[1]], ["desc"]).map(([weight, count]) => ({
					weight,
					count
				}))
			};
