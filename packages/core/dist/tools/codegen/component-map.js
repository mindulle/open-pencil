...
//#region src/tools/codegen/component-map.ts
function collectInstanceCounts(figma, componentIds) {
	const counts = /* @__PURE__ */ new Map();
	figma.currentPage.findAll((node) => {
		if (node.type !== "INSTANCE") return false;
		const raw = figma.graph.getNode(node.id);
		if (!raw?.componentId) return false;
		if (componentIds.has(raw.componentId)) counts.set(raw.componentId, (counts.get(raw.componentId) ?? 0) + 1);
		return false;
	});
	return counts;
}
function detectPropCandidates(node, graph) {
	const props = [];
	for (const childId of node.childIds) {
		const child = graph.getNode(childId);
		if (!child) continue;
		if (child.type === "TEXT" && child.text) props.push(`text:${child.name}`);
		if (child.fills.some((fill) => fill.visible && fill.type === "SOLID")) {
			if (Object.keys(child.boundVariables).length > 0) props.push(`color:${child.name}`);
		}
		if (child.type === "FRAME" || child.type === "GROUP") props.push(`slot:${child.name}`);
	}
	return props;
}
function detectVariants(componentNode, graph) {
...
function buildScreenInfo(figma, frameNode, componentIds) {
	...
	const walk = (nodeId) => {
		...
		if (node.type === "INSTANCE" && node.componentId && componentIds.has(node.componentId)) {
			...
			if (entry) entry.count++;
				...
				refs.set(node.componentId, {
					name: component?.name ?? node.componentId,
					count: 1
				});
			}
		}
	...
	return {
		...
		componentRefs: [...refs.entries()].map(([id, { name, count }]) => ({
			id,
			name,
			count
		})),
...
const designToComponentMap = defineTool({
	name: "design_to_component_map",
	description: "Analyze the document and return a structured component decomposition: components (with variants, props, instance counts), screens, and a dependency overview.",
	params: { page: {
		type: "string",
	...
	execute: (figma, args) => {
		...
		for (const component of components) component.instanceCount = instanceCounts.get(component.id) ?? 0;
		const screens = [];
		...
		for (const child of page.children) {
			const raw = figma.graph.getNode(child.id);
			if (raw?.type === "SECTION") sections.push({
				id: raw.id,
				name: raw.name,
