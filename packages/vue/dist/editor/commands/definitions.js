...
//#region src/editor/commands/edit.ts
function createEditCommands({ editor, capabilities, messages: t }) {
...
//#endregion
//#region src/editor/commands/selection.ts
function selectedMaskTarget(nodes, editor) {
	if (nodes.length === 0) return null;
	if (nodes.length === 1) return nodes[0];
	const [first] = nodes;
	const byId = new Map(nodes.map((node) => [node.id, node]));
	if (new Set(nodes.map((node) => node.parentId)).size !== 1) return first;
...
function createSelectionCommands({ editor, selection, capabilities, messages: t, otherPages, moveSelectionToPage, getOpacityTarget }) {
	return {
		"selection.selectAll": {
		...
		"selection.toggleMask": {
			...
			get label() {
				return selectedMaskTarget(editor.getSelectedNodes(), editor)?.isMask ? t.value.removeMask : t.value.useAsMask;
			},
			enabled: capabilities.canToggleMask,
			run: () => {
				const target = selectedMaskTarget(editor.getSelectedNodes(), editor);
				if (!target) return;
				editor.updateNodeWithUndo(target.id, { isMask: !target.isMask }, target.isMask ? "Remove mask" : "Use as mask");
			}
		},
...
//#endregion
//#region src/editor/commands/view.ts
function createViewCommands({ editor, capabilities, messages: t }) {
	return {
		"view.zoom100": {
			id: "view.zoom100",
			get label() {
...
//#endregion
//#region src/editor/commands/definitions.ts
function createEditorCommandMap(options) {
	return {
