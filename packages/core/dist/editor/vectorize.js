import { createVectorFrameChildren, resolveVectorFramePlacement } from "../vector/vectorize/placement.js";
...
//#region src/editor/vectorize.ts
function hasRoundedCorners(node) {
	...
	].some((radius) => radius > 0) : node.cornerRadius > 0;
}
function replacementFrameProps(node) {
	return {
		name: node.name,
...
function createVectorizeActions(ctx) {
	function replaceNodeWithVectorFrame(nodeId, vectorized) {
		...
		ctx.setSelectedIds(/* @__PURE__ */ new Set([frame.id]));
		ctx.undo.push({
