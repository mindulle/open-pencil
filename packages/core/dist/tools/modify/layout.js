...
//#region src/tools/modify/layout.ts
const setLayout = defineTool({
	...
	execute: (figma, args) => {
		...
		if (!args.direction && raw?.layoutMode === "NONE") return { error: "Frame has no auto-layout. Pass direction (\"HORIZONTAL\" or \"VERTICAL\") to enable it." };
		...
		if (args.spacing !== void 0) node.itemSpacing = args.spacing;
		if (args.align !== void 0) node.primaryAxisAlignItems = args.align;
		if (args.counter_align !== void 0) node.counterAxisAlignItems = args.counter_align;
		if (args.flow_direction !== void 0) node.layoutDirection = args.flow_direction;
		if (args.padding !== void 0) {
			node.paddingTop = args.padding;
			node.paddingRight = args.padding;
			node.paddingBottom = args.padding;
			node.paddingLeft = args.padding;
		}
		if (args.padding_horizontal !== void 0) {
			node.paddingLeft = args.padding_horizontal;
			node.paddingRight = args.padding_horizontal;
		}
		if (args.padding_vertical !== void 0) {
...
const setConstraints = defineTool({
	...
	execute: (figma, args) => {
		const node = figma.getNodeById(args.id);
		if (!node) return nodeNotFound(args.id);
		if (args.horizontal || args.vertical) node.constraints = {
...
const setLayoutChild = defineTool({
	name: "set_layout_child",
	mutates: true,
	description: "Configure auto-layout child: sizing (FIXED/HUG/FILL), grow, alignment, absolute positioning.",
	params: {
		id: {
			type: "string",
		...
		grow: {
			type: "number",
			description: "Flex grow factor (0 = fixed, 1 = grow)",
			min: 0
		},
		align_self: {
			type: "string",
...
//#endregion
export { setConstraints, setLayout, setLayoutChild };
