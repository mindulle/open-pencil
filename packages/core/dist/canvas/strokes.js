...
//#region src/canvas/strokes.ts
function getStrokeCapEntity(r, cap) {
	switch (cap) {
...
function getStrokeJoinEntity(r, join) {
	switch (join) {
...
function strokeInset(stroke) {
	if (stroke.align === "INSIDE") return stroke.weight / 2;
	if (stroke.align === "OUTSIDE") return -stroke.weight / 2;
	return 0;
}
function drawDashedRRectWithSolidCorners(r, canvas, node, stroke, color, cornerRadius, dashPhase = 0) {
	...
	canvas.drawArc(r.ck.LTRBRect(left, top, left + radius * 2, top + radius * 2), 180, 90, false, r.strokePaint);
	canvas.drawArc(r.ck.LTRBRect(right - radius * 2, top, right, top + radius * 2), 270, 90, false, r.strokePaint);
	canvas.drawArc(r.ck.LTRBRect(right - radius * 2, bottom - radius * 2, right, bottom), 0, 90, false, r.strokePaint);
	canvas.drawArc(r.ck.LTRBRect(left, bottom - radius * 2, left + radius * 2, bottom), 90, 90, false, r.strokePaint);
...
function configureStrokePaint(r, node, stroke, color) {
	r.strokePaint.setColor(r.ck.Color4f(color.r, color.g, color.b, color.a));
	r.strokePaint.setStrokeWidth(stroke.weight);
	r.strokePaint.setAlphaf(stroke.opacity);
...
function drawStyledRRectStroke(r, canvas, rrect, node, stroke, color, dashPhase = 0) {
	const dash = stroke.dashPattern ?? [];
...
function drawNodeStroke(r, canvas, node, rect, hasRadius) {
	switch (node.type) {
		case "VECTOR": {
			const vps = r.getVectorPaths(node);
			if (vps) for (const vp of vps) canvas.drawPath(vp, r.strokePaint);
			break;
		}
		case "ELLIPSE": {
			const fg = r.getFillGeometry(node);
			if (fg) for (const p of fg) canvas.drawPath(p, r.strokePaint);
			else if (node.arcData) r.drawArc(canvas, node, r.strokePaint);
			else canvas.drawOval(rect, r.strokePaint);
			break;
		}
		case "POLYGON":
		case "STAR": {
		...
		default: if (nodeHasSmoothCorners(node)) {
			...
			canvas.drawPath(path, r.strokePaint);
			path.delete();
		} else if (hasRadius) canvas.drawRRect(r.makeRRect(node), r.strokePaint);
		else canvas.drawRect(rect, r.strokePaint);
	}
}
function drawStrokeWithAlign(r, canvas, node, rect, hasRadius, align) {
	if (align === "INSIDE") {
		canvas.save();
		...
		r.strokePaint.setStrokeWidth(origWidth);
		canvas.restore();
	} else r.drawNodeStroke(canvas, node, rect, hasRadius);
}
function drawRRectStrokeWithAlign(r, canvas, rrect, node, stroke) {
	...
	if (stroke.align === "INSIDE") {
		...
		r.strokePaint.setStrokeWidth(stroke.weight);
		canvas.restore();
	} else canvas.drawRRect(rrect, r.strokePaint);
}
...
function strokeNodeShape(r, canvas, node, paint) {
	...
	switch (node.type) {
		...
		case "VECTOR": {
			const vps = r.getVectorPaths(node);
			if (vps) for (const vp of vps) canvas.drawPath(vp, paint);
			return;
		}
		case "LINE":
	...
	if (node.cornerRadius > 0 || node.independentCorners && (node.topLeftRadius > 0 || node.topRightRadius > 0 || node.bottomRightRadius > 0 || node.bottomLeftRadius > 0)) if (node.independentCorners) {
	...
	} else canvas.drawRRect(r.ck.RRectXY(rect, node.cornerRadius, node.cornerRadius), paint);
