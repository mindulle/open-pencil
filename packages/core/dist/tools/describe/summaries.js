...
//#region src/tools/describe/summaries.ts
function boundFillSuffix(node, fillIndex, graph) {
...
function findSolidFillIndex(node) {
...
function describeVisual(node, graph) {
	const parts = [];
...
function describeLayout(node) {
	...
	const parts = [node.layoutMode === "HORIZONTAL" ? "horizontal" : "vertical"];
...
function summarizeContainer(node, graph) {
...
function summarizeText(node, graph) {
	let summary = `"${node.text.slice(0, 60)}" ${node.fontSize}px ${node.fontFamily}`;
