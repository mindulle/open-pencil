...
//#region src/tools/analyze/clusters.ts
function calcClusterConfidence(nodes) {
	...
	for (const node of nodes.slice(1)) {
		...
		else if (sizeDiff <= 10 && childDiff <= 1) score += .8;
		else if (sizeDiff <= 20 && childDiff <= 2) score += .6;
		else score += .4;
	}
	return Math.round(score / (nodes.length - 1) * 100);
}
const analyzeClusters = defineTool({
	...
	params: {
		min_count: {
			type: "number",
			description: "Min instances to form a cluster (default: 2)"
		},
		min_size: {
			type: "number",
			description: "Min node size in px (default: 30)"
		},
		limit: {
			type: "number",
	...
	execute: (figma, args) => {
		...
		page.findAll((node) => {
			...
			const childSignature = sortBy([...childTypes.entries()], [([type]) => type]).map(([type, count]) => `${type}:${count}`).join(",");
		...
		const clusters = orderBy([...signatureMap.entries()].filter(([, nodes]) => nodes.length >= minCount).map(([signature, nodes]) => {
