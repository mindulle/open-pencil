import { colorToHex, parseColor } from "../../color/index.js";
import { defineTool } from "../schema.js";
import { safeDestr } from "destr";
import { createTwoFilesPatch } from "diff";
//#region src/tools/analyze/diff.ts
...
function serializeCornerRadii(raw, lines) {
	...
	if (tl || tr || br || bl) if (tl === tr && tr === br && br === bl) lines.push(`radius: ${tl}`);
	else lines.push(`radii: ${tl} ${tr} ${br} ${bl}`);
}
...
function collectNodeTree(figma, nodeId, parentPath, depth, maxDepth) {
	...
	result.set(path, {
		path,
		id: raw.id,
		serialized: serializeNodeProps(raw)
	});
	if (depth < maxDepth) for (const childId of raw.childIds) {
...
function createUnifiedDiff(oldFilename, newFilename, oldContent, newContent) {
	return createTwoFilesPatch(oldFilename, newFilename, oldContent, newContent, "", "").split("\n").filter((l) => !l.startsWith("Index:") && l !== "===================================================================").join("\n").trim();
}
const diffCreate = defineTool({
	name: "diff_create",
	...
	execute: (figma, args) => {
		...
		const allPaths = /* @__PURE__ */ new Set([...fromNodes.keys(), ...toNodes.keys()]);
		const patches = [];
		for (const path of allPaths) {
			...
			} else if (fromEntry && toEntry && fromEntry.serialized !== toEntry.serialized) patches.push(createUnifiedDiff(`${path} #${fromEntry.id}`, `${path} #${toEntry.id}`, fromEntry.serialized, toEntry.serialized));
		}
...
const diffShow = defineTool({
	...
	execute: (figma, args) => {
		...
		let newProps;
		try {
			newProps = safeDestr(args.props);
		} catch {
			return { error: "Invalid JSON in props" };
		}
		const modified = structuredClone(raw);
		if (newProps.fill) modified.fills = [{
			type: "SOLID",
			color: parseColor(newProps.fill),
			opacity: 1,
			visible: true
		}];
		if (newProps.stroke) modified.strokes = [{
		...
		if (newProps.opacity !== void 0) modified.opacity = Number(newProps.opacity);
		if (newProps.radius !== void 0) {
		...
		if (newProps.width !== void 0) modified.width = Number(newProps.width);
		if (newProps.height !== void 0) modified.height = Number(newProps.height);
		if (newProps.x !== void 0) modified.x = Number(newProps.x);
		if (newProps.y !== void 0) modified.y = Number(newProps.y);
		if (newProps.visible !== void 0) modified.visible = Boolean(newProps.visible);
		if (newProps.locked !== void 0) modified.locked = Boolean(newProps.locked);
		if (newProps.rotation !== void 0) modified.rotation = Number(newProps.rotation);
		if (newProps.text !== void 0) modified.text = newProps.text;
