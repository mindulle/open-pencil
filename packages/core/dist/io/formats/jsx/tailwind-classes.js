...
import { colorToCSSCompact } from "../../../color/index.js";
import { formatTrack, getNodeContext, solidFillColor, solidStroke } from "./helpers.js";
import { twirl } from "twirlwind";
//#region src/io/formats/jsx/tailwind-classes.ts
function px(v) {
	return `${v}px`;
}
function gridTemplateTw(tracks) {
	if (tracks.every((t) => t.sizing === "FR" && t.value === 1)) return String(tracks.length);
	return `[${tracks.map(formatTrack).join("_")}]`;
}
function collectGridClasses(node) {
	const classes = ["grid"];
	if (node.gridTemplateColumns.length > 0) classes.push(`grid-cols-${gridTemplateTw(node.gridTemplateColumns)}`);
	if (node.gridTemplateRows.length > 0) classes.push(`grid-rows-${gridTemplateTw(node.gridTemplateRows)}`);
	return classes;
}
...
function applyFlexStyle(style, node) {
	...
	if (node.layoutWrap === "WRAP" && node.counterAxisSpacing > 0) style.rowGap = px(node.counterAxisSpacing);
...
function applyPadding(style, node) {
...
function applyAppearanceStyle(style, node) {
	...
	if (node.cornerRadius > 0) if (node.independentCorners) style.borderRadius = `${px(node.topLeftRadius)} ${px(node.topRightRadius)} ${px(node.bottomRightRadius)} ${px(node.bottomLeftRadius)}`;
	else style.borderRadius = node.cornerRadius >= 9999 ? "9999px" : px(node.cornerRadius);
	...
	for (const effect of node.effects) {
		...
		} else if (effect.type === "LAYER_BLUR" || effect.type === "FOREGROUND_BLUR") style.filter = `blur(${px(effect.radius)})`;
		else style.backdropFilter = `blur(${px(effect.radius)})`;
	}
}
function applyTextStyle(style, node) {
	if (node.type !== "TEXT") return;
	style.fontSize = px(node.fontSize);
	if (node.fontFamily && node.fontFamily !== "Inter") style.fontFamily = node.fontFamily;
...
function collectTailwindClasses(node, graph) {
	...
	if (node.type === "TEXT" && resolveNodeTextDirection(node) === "RTL") extraClasses.push("[direction:rtl]");
...
//#endregion
export { collectTailwindClasses };

//# sourceMappingURL=tailwind-classes.js.map